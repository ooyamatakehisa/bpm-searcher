...
class PlaylistController:
    ...
    def update_playlist(self, uid: str, playlist_id: str) -> Response:
        ...
        if body["kind"] == "info":
            ...
            exist = self.playlist_usecase.update_playlist(
                kind=body["kind"],
    ...
    def delete_track(self, uid: str, playlist_id: str, playlist_track_id: str):
        headers = request.headers
        bearer = headers.get("Authorization")
        if bearer is None:
            return make_response("no id token is specified", 401)
        ...
        exist = self.playlist_usecase.delete_track(playlist_id, playlist_track_id)
        if not exist:
